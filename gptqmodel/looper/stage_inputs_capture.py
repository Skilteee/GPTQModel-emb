...
class StageInputsCapture:
    ...
    def cache_inputs_embed(self, calibration_data):
        ...
        for i in range(len(calibration_data)):
            layer_inputs.append([calibration_data[i]['embed_inputs']])
            attention_masks.append(None)

            position_id = torch.arange(
                calibration_data[i]['embed_inputs'].shape[1],
    ...
    def get_emb(self, calibration_data):

        for each in calibration_data:
            input_ids = each['input_ids']
            embed_inputs = self.gptq_model.turtle_model.model.embed_tokens(input_ids)
            each['embed_inputs'] = embed_inputs
    ...
    def cache_inputs(
        ...
        # calibration_data = self.get_emb(calibration_data)
        if 'embed_inputs' in calibration_data[0]:
            result = self.cache_inputs_embed(calibration_data)
